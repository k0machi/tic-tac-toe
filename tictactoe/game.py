...
class Game:
    ...
    def _check_line(self, candidate, ply_type) -> Tuple[bool, Union[Point, X, O]]:
        hits = [cell for cell in self._board.flat_view() 
            if type(cell) is ply_type and [cell.x, cell.y] in candidate
        ]
    ...
    def _check(self) -> Tuple[bool, Union[Point, X, O]]:
        ...
        for winstate in self.candidate_wins:
            for player in self.players:
                win, player = self._check_line(
                    candidate=winstate, ply_type=player.symbol)
                if win:
                    return (True, player)
    ...
    def run(self) -> bool:
        for player in cycle(self._players):
            ...
            if win:
                player_name = [
                    ply.name for ply in self.players if ply.symbol == player_type
                ].pop()
                print("Board:", self._board, sep="\n")
